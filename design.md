...
- **Mobile/tablet (below `md`):** the sidebar is hidden and a fixed bottom tab bar (`BottomNav`) takes its place, per the mobile-first confirmed direction above — same four items and icons, icon above label, the active tab fully accent-colored. A condensed top bar (the same `Wordmark` component, `size="compact"`) replaces the sidebar's wordmark placement so the brand mark stays visible with the sidebar gone.
...
- `Wordmark` — MOGᴰ wordmark + accent underline; `size="default"` (sidebar) or `"compact"` (mobile top bar).
- `SidebarNav` — `Wordmark` + icon nav list as described above; hidden below `md`.
